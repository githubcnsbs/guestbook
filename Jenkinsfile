...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="cnsbs2010/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/githubcnsbs/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'Dockerhub_cnsbs2010') {
                        oDockImage.push()
                    }
                }
            }
    ...
    post { 
        ...
        success { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) 빌드가 성공적으로 끝났습니다21. Details: (<${BUILD_URL} | here >)")
        }
        failure { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
